...
export async function PATCH(request: NextRequest) {
    ...
    */
    ...
    if (!rolle || !user.id) {
        return NextResponse.json({ error: "Rolle and userID are required" }, { status: 400 });
    }
    try {
        ...
        if (!existingRolle) {
            return NextResponse.json({ error: "Rolle and userID combination not found" }, { status: 404 });
        }
        ...
        return NextResponse.json({ message: "Rolle successfully updated", rolle: updatedRolle }, { status: 200 });
    } catch (error) {
        console.error("Error updating rolle in database:", error);
        return NextResponse.json({ message: "Internal server error. Could not update rolle." }, { status: 500 });
    }
}
...
export async function DELETE (request: NextRequest){
    ...
    const rolle = url.searchParams.get('rolle');
    if (!rolle){
        return NextResponse.json({error: "Rolle name is required"},{status:400});
    }
    try{
        await db.hovedstyret.delete({
            where: {rolle: rolle},
        });
        return NextResponse.json({message: "Rolle successfully deleted"}, {status: 200});
    }
    catch(error){
