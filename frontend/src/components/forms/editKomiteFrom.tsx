...
const EditKomiteForm: React.FC<leggTilKomiteFormProps> = ({
  komite,
  handleCloseForm,
}) => {
  const [komiteID, setKomiteID] = useState<string>(komite.id ?? "");
  ...
  const saveData = async (komite: Komite) => {
    try {
      const response = await fetch(`/api/komite/${komiteID}`, {
        method: "PATCH",
        headers: {
      ...
      if (response.status !== 200) {
        ...
        console.log("Successfully updated komite");
      }
    } catch (error) {
  ...
  return (
    <form onSubmit={handleFormSubmit} className="flex flex-col gap-4 p-4">
      <h2 className="text-xl font-bold">Rediger komite</h2>
