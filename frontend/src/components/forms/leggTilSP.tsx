...
const LeggTilSPForm: React.FC<leggTilSPFormProps> = ({ handleCloseForm }) => {
  ...
  const saveData = async (SP: Samarbeidspartner) => {
    try {
      const response = await fetch(`/api/samarbeidspartner`, {
        method: "POST",
        headers: {
      ...
      if (response.status !== 200) {
        ...
        console.log("Successfully added new SP");
      }
    } catch (error) {
