...
interface editHSPFormProps {
  handleCloseForm: () => void;
  HSP: Hovedsamarbeidspartner;
}
const EditHSPForm: React.FC<editHSPFormProps> = ({ handleCloseForm, HSP }) => {
  const [formData, setFormData] = useState<Hovedsamarbeidspartner>({
    navn: HSP.navn,
    beskrivelse: HSP.beskrivelse,
    active: HSP.active,
    logo: HSP.logo,
    hjemmeside: HSP.hjemmeside,
    annonseside: HSP.annonseside,
    id: HSP.id,
  });
  ...
  const saveData = async (HSP: Hovedsamarbeidspartner) => {
    try {
      const response = await fetch(`/api/hovedsamarbeidspartner/${HSP.id}`, {
        method: "PATCH",
        headers: {
      ...
      if (response.status !== 200) {
        ...
        console.log(`Successfully updated ${HSP.navn}`);
      }
    } catch (error) {
