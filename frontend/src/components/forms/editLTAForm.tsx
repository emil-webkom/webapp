...
interface editLTAForm {
  handleCloseForm: () => void;
  LTA: lavTerskelArrangement;
}
const EditLTAForm: React.FC<editLTAForm> = ({ handleCloseForm, LTA }) => {
  ...
  const saveData = async (SP: lavTerskelArrangement) => {
    try {
      const response = await fetch(`/api/lavterskelarrangement/${LTA.id}`, {
        method: "PATCH",
        headers: {
      ...
      if (response.status !== 200) {
        ...
        console.log("Successfully updated lavterskelarrangement");
      }
    } catch (error) {
